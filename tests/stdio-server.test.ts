...
describe("qwen-2 stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "qwen-2-mcp-test", version: "0.1.0" });
    ...
    for (const model of ["qwen-2-edit-image","qwen-2-remix-image","qwen-2-text-to-image"]) {
